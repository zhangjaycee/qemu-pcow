                            ...
                            Error **errp)
{
    ...
    area = qemu_ram_mmap(fd, memory, block->mr->align,
                         block->flags & RAM_SHARED, block->mr->format);
    if (area == MAP_FAILED) {
        error_setg_errno(errp, errno,
    ...
    if (mem_prealloc) {
        ...
        if (errp && *errp) {
            qemu_ram_munmap(area, memory, block->mr->format);
            return NULL;
        }
    }
                                 ...
                                 Error **errp)
{
    ...
    // by zjc:
    if (mr->format == MEM_BACKEND_FORMAT_PCOW) {
        struct Super *s = read_first_super(fd, 0, 0);
        if (NULL == s) {
            printf("File is not fomatted!\n");
            return MAP_FAILED;
        }
        size = (uint64_t) s->max_block_num * s->block_size;
        printf("now in exec.c size: %ld\n", size);
        free(s);
    } else if(file_size > 0 && file_size < size) {
...
static void reclaim_ramblock(RAMBlock *block)
{
    if (block->flags & RAM_PREALLOC) {
        ...
        qemu_ram_munmap(block->host, block->max_length, block->mr->format);
        close(block->fd);
#endif
    } else {
